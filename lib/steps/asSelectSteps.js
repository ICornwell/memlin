    ...
    return function (args) {
      ...
      var ts = context.traversers.map(function (t) {
        var sels = labels.map(function (l, i) {
          ...
          var useByName = args.by && i <= args.by.length && sel[args.by[i]];
          var selBy = useByName ? sel[args.by[i]] : sel;
          return idx >= 0 ? _defineProperty({}, l, selBy) : undefined;
        }).filter(function (obj) {
          return obj;
        });
...
var by = function by(_by) {
  return function (getCurrentContext) {
    return function (args) {
      _by = ensureIsArray(_by);
      var bys = agrs.by ? [].concat(_toConsumableArray(_by), _toConsumableArray(args.by)) : _by;
