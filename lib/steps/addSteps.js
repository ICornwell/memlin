  ...
  return function (getCurrentContext) {
    return function (args) {
      ...
      var vs = context.traversers.filter(function (t) {
        return (0, _traverser.isV)(t.objects[0]);
      });
...
var to = function to(_to) {
  return function (getCurrentContext) {
    return function (args) {
      _to = resolveTraverserArg(_to, args);
      _to = ensureIsArray(_to);
...
function resolveTraverserArg(arg, context) {
  if (!(arg.subQuery && {}.toString.call(arg.subQuery) === '[object Function]')) return arg;
  if (arg.subQuery) return arg.subQuery(context);
}