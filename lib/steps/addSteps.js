...
function sanitise(val) {
  if (typeof val === 'string') return val.replace(/[\\]/g, "\\\\'").replace(/[']/g, "\\'");else return val;
}
...
var addV_Text = function addV_Text(label, props) {
  ...
  if (props) Object.keys(props).forEach(function (p) {
    if (props[p]) if (_typeof(props[p]) === 'object') {
      ...
      steps.push("property(list, '".concat(p, "', ").concat(keyValsList, ")"));
    } else steps.push("property('".concat(p, "', '").concat(sanitise(props[p]), "')"));
  });
  return steps.join('.');
};
